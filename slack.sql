...
SELECT * 
	FROM `studentsubject` AS `s`
		LEFT JOIN (SELECT `sid`, MAX(`mark`) AS `max_mark` 
			FROM `studentsubject` GROUP BY `sid`) AS `s2`
				ON `s`.`sid` = `s2`.`sid` AND `s`.`mark` = `s2`.`max_mark` 
					ORDER BY `mark` DESC LIMIT 0, 2;
-- 18. Hiển thị ngày thi có điểm thi cao nhất.
SELECT `date`, `mark` FROM `studentsubject` WHERE `mark` = ANY (SELECT MAX(`mark`) FROM `studentsubject`);